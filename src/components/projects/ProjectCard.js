...
export default function ProjectCard({ project, span }) {
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true }}
      className={`${span} rounded-[28px] overflow-hidden border border-white/10 bg-white/[0.04]`}
    >
      {/* IMAGE */}
      ...
      <div className="p-8">
        ...
        <p className="text-gray-400 mb-6 max-w-[720px]">
          {project.description}
        </p>
        ...
        <div className="flex flex-wrap gap-2 mb-6">
          {project.tech?.map((tech) => (
            <span
              key={tech}
              className="px-4 py-1 text-xs rounded-full bg-white text-black"
            >
              {tech}
            </span>
          ))}
